...
<CyberCard
  class="overflow-hidden p-0 relative h-100 border border-primary/30"
  glowState="primary"
>
  <div class="absolute inset-0 z-0">
    <div
      class="absolute inset-0 bg-linear-to-r from-[#0B0C10] via-[#0B0C10]/80 to-transparent z-10"
    ></div>
    <img
      src={imageSrc}
      alt={title}
      class="h-full w-full object-cover opacity-60 mix-blend-screen"
    />
  </div>
  ...
  <div class="relative z-20 p-8 h-full flex flex-col justify-center max-w-xl">
    ...
    {#if startDate}
      <div
        class="flex items-center gap-3 text-xs font-mono text-secondary mb-6 uppercase tracking-widest font-semibold"
      >
    ...
    <div class="flex gap-4">
      {#if journeyId}
        <Button
          ...
          class="px-8 py-6 rounded-none bg-primary text-primary-foreground hover:bg-primary/90 font-bold tracking-widest shadow-[0_0_20px_rgba(255,42,122,0.6)]"
        >
          RESUME SESSION
        </Button>
      {:else}
        <Button
          href="/trips/{tripSlug || 'trip'}/{tripId}"
          class="px-8 py-6 rounded-none bg-primary text-primary-foreground hover:bg-primary/90 font-bold tracking-widest shadow-[0_0_20px_rgba(255,42,122,0.6)]"
        >
          VIEW DETAILS
        </Button>
